Ext.define('CoM.view.isaSimPanelGrid', {
    extend: 'Ext.grid.Panel',
    alias: 'widget.isaSimPanelGrid',
	...
	plugins: [{
        ptype: 'rowexpander',
        pluginId: 'isaSimPanelGridrowexpanderID',
				...
				if (record.get('simstate')== 2 && typeof Ext.getCmp("resultBasicLayerGridID@"+targetId+"@"+rowIdx) === "undefined"){
					...
					infos = simoutput.infos;
						...
						for(var j=0; j < vectors.length; j++){
							...
							if (vectors[j].layertype == "polygon" && vectors[j].layerclass == "suc")
							{
								for(var k=0; k<infos.length; k++){
									...
									layerinfo = new CoM.model.modelLayerInfo({
													...
													layerconfig:{
														url:vectors[j].gsurl,
														servicetype:'WFS',
														colorValueField: 'suc_'+infos[k].layernameabbr+"_r",
														colorRamp:'white-red',
							...
							} else if (vectors[j].layertype == "polygon" && vectors[j].layerclass == "isa"){
								for(var k=0; k<infos.length; k++){
									var displayname = vectors[j].layerclass+"_"+infos[k].layernameabbr;
									layerinfo = new CoM.model.modelLayerInfo({
													layerdisplayname: displayname,
													...
													layerconfig:{
														url:vectors[j].gsurl,
														servicetype:'WFS',
														colorValueField: 'isa_'+infos[k].layernameabbr+"_r",
														colorRamp:'white-red',
														iconurl:'images/'+vectors[j].layertype+'.png'
													},
													showdownload:true
									...
									Ext.Ajax.request({
													   ...
													   success: function(response, opts) {
														  ...
														  if(obj.vectors.length>0){
															messageHelper.showNotificationWait("ISA: Finished. <br/>Click the + button to explore model outputs.", 3000);
															//store result into store and update sim state
															//TODO: another thought is to separate each service isa/netbuf into different rows. 
															grid.getStore().getAt(rowIndex).set('simoutput',obj);
															grid.getStore().getAt(rowIndex).set('simstate',2);
														  }else
														  {
															messageHelper.showErrorWait("ISA: Failed. Empty outputs returned.<br/>Reason: Server exception occurs.<br/>Please try it again.", 3000);
													   ...
													   failure: function(response, opts) {
														  ...
														  messageHelper.showErrorWait("ISA: Failed. <br/>Reason: "+response.responseText+"<br/>Please try it again.", 3000);
														  grid.getStore().getAt(rowIndex).set('simstate',3);
													   }
													});
								},
