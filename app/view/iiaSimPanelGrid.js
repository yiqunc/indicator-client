Ext.define('CoM.view.iiaSimPanelGrid', {
    extend: 'Ext.grid.Panel',
    alias: 'widget.iiaSimPanelGrid',
	...
	plugins: [{
        ptype: 'rowexpander',
        pluginId: 'iiaSimPanelGridrowexpanderID',
        ...
        toggleRow: function(rowIdx) {
            ...
            if (row.hasCls(this.rowCollapsedCls)) {
				...
				if (record.get('simstate')== 2 && typeof Ext.getCmp("resultBasicLayerGridID@"+targetId+"@"+rowIdx) === "undefined"){
					...
					infos = simoutput.infos;
					enableUPQ = record.get('simpara').enableUPQ;
					enableRankAmongAllLGAs = record.get('simpara').enableRankAmongAllLGAs;
					iiaMinMeshBlockPopThreshold = record.get('simpara').iiaMinMeshBlockPopThreshold;
					
					if(typeof enableUPQ === "undefined") enableUPQ = false;
					if(typeof enableRankAmongAllLGAs === "undefined") enableRankAmongAllLGAs = false;
					if(typeof iiaMinMeshBlockPopThreshold === "undefined") iiaMinMeshBlockPopThreshold = 0;
						...
						for(var j=0; j < vectors.length; j++){
							...
							if (vectors[j].layertype == "polygon" && vectors[j].layerclass == "iia")
							{
								...
								if(enableRankAmongAllLGAs){
									displayname = "iiag-rank among LGAs";
									layerinfo = new CoM.model.modelLayerInfo({
													layerdisplayname: displayname,
													layername: vectors[j].name+"_amongLGAs", 
													layertype: vectors[j].layertype,
													layervisible: false, 
													plname: '',
													plgeometrytype: '',
									...
									if(enableUPQ){
										displayname = "upqg-rank among LGAs";
										layerinfo = new CoM.model.modelLayerInfo({
															layerdisplayname: displayname,
															layername: vectors[j].name+"_upq_amongLGAs", 
															layertype: vectors[j].layertype,
															layervisible: false, 
															plname: '',
															plgeometrytype: '',
							...
							} else if (vectors[j].layertype == "polygon" && vectors[j].layerclass == "filteredMB"){
								
									var displayname = "filtered MB (pop<"+iiaMinMeshBlockPopThreshold+")";
									layerinfo = new CoM.model.modelLayerInfo({
													layerdisplayname: displayname,
													layername: vectors[j].name, 
													layertype: vectors[j].layertype,
							...
							} else if (vectors[j].layertype == "polygon" && vectors[j].layerclass == "netbuf"){
								for(var k=0; k < infos.length; k++){
									var displayname = vectors[j].layerclass+"_"+infos[k].layernameabbr;
									layerinfo = new CoM.model.modelLayerInfo({
													layerdisplayname: displayname,
													layername: vectors[j].name+"_"+infos[k].layernameabbr, 
													layertype: vectors[j].layertype,
													layervisible: false, 
													plname: '',
													plgeometrytype: '',
													plgeometrycount: -1,
													layerconfig:{
														url:vectors[j].gsurl+"&cql_filter=servicetyp='"+infos[k].layernameabbr+"'",
														servicetype:'WFS',
									...
									Ext.Ajax.request({
													   ...
													   success: function(response, opts) {
														  ...
														  if(obj.vectors.length>0){
															messageHelper.showNotificationWait("IIA: Finished. <br/>Click the + button to explore model outputs.", 3000);
														  ...
														  {
															messageHelper.showErrorWait("IIA: Failed. Empty outputs returned.<br/>Reason: Server exception occurs.<br/>Please try it again.", 3000);
													   ...
													   failure: function(response, opts) {
														  ...
														  messageHelper.showErrorWait("IIA: Failed. <br/>Reason: "+response.responseText+"<br/>Please try it again.", 3000);
														  grid.getStore().getAt(rowIndex).set('simstate',3);
													   }
													});
								},
